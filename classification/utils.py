...
def build_model(input_dim,class_num,weight_decay = 0.0001,dense_units=128):
    model = Sequential()
    model.add(Dense(4*dense_units,input_dim=input_dim, kernel_initializer='he_normal', kernel_regularizer=l2(weight_decay),activation='relu'))
    ...
    model.add(Dense(dense_units, kernel_initializer='he_normal', kernel_regularizer=l2(weight_decay),activation='relu'))
    model.add(Dropout(0.25))
    model.add(Dense(class_num, kernel_initializer='he_normal', kernel_regularizer=l2(weight_decay)))
